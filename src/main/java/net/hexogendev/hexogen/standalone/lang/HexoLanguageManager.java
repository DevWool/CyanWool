...
public class HexoLanguageManager implements ILanguageManager {
	...
	public HexoLanguageManager(Server server) throws IOException {
		this.server = server;
	...
	public void removeLanguageFile(String langCode) {
		if (hasAvailableLanguage(langCode)) {
			ILanguageFile lang = languages.get(langCode);
			lang.save();
	...
	public void loadAllLanguages() {
		...
		for (File file : langList) {
			...
			ILanguageFile lf = new HexoLanguageFile(fileName, file);
	...
	public ILanguageFile getLanguageFile(String isoCode) {
		ILanguageFile file = languages.get(isoCode);
		if (file == null) {
			file = languages.get("en_US");
		}
		return file;
	}
